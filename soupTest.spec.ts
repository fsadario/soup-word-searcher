...
describe('alphabetSoup', () => {
  ...
  beforeAll(() => {
    searcher = new WordSearcher(soup);
  });

  describe.each(INSOUP)('with word %s', (word: string) => {
    test('IN SOUP', () => {
      expect(searcher.isPresent(word)).toBeTruthy();
    });
  });

  describe.each(NOTINSOUP)('with word %s', (word: string) => {
    test('NOT IN SOUP', () => {
      expect(searcher.isPresent(word)).toBeFalsy();
    });
  });
});
